...
public class TokenReValidationWorker extends RxWorker {
    ...
    public TokenReValidationWorker(@NonNull Context context, @NonNull WorkerParameters workerParams) {
                                    ...
                                    if (loginResponse.getFacilities().size() > 0) {
                                        try{
                                            repository.insert(loginResponse.getFacilities())
                                                    ...
                                                    .doOnError(throwable -> {
                                                        Log.e("Errrrrrr", throwable.getMessage());
...
//                    Toast.makeText(mContext, "Failed poooooooo", Toast.LENGTH_LONG).show();
    ...
    private void processLoginResponse(LoginResponseSingle loginResponse) {
        if (mManager.isAccountDeactivated()) {
            ...
            mManager.setEnrolmentLimit(loginResponse.getEnrolmentLimit());
            mManager.setLGAEnrolmentLimit(loginResponse.getLgaLimit());
